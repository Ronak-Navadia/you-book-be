...
export const loginUser = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ code: 1, message: "Login successful", data: userData });
  } catch (error) {
    console.error(error);
    res.status(500).json({ code: 0, message: error.message });
  }
...
export const forgotPassword = async (req, res) => {
  ...
  try {
    ...
    // Mock sending email
    await sendMail(email, "Reset Your Password", "verification-code.template", {
      code,
    });
...
export const verifyCode = async (req, res) => {
  ...
  try {
    ...
    if (!savedCode || savedCode !== code) {
      return res
        .status(400)
        .json({ code: 0, message: "Invalid or expired verification code" });
    }
